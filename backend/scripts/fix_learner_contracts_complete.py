    ...
    try:
        ...
        for t in topics:
            if not t.estimated_minutes:
                t.estimated_minutes = 30  # Default
                updated += 1
        print(f"Updated estimated_minutes for {updated} topics")

        # 2. Fix resource boundaries
        from app.content.learner_visibility import is_learner_visible
        ...
        # 3. Fix resource metadata mismatches
        meta_fixed = 0
        ...
        # 5. Ensure practice contracts exist for all substantive units
        practice_added = 0
        from app.db.models import LessonExercise
